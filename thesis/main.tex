...
data Frame : Type → Type → Set where
    ...
    _⊞₀_     : ⊨ Nat → ⊢ Nat → Frame Nat Nat 
    _⊞₁_     : ⊨ Nat → Frame Nat Nat
...
We extend the arithmetic language with exceptions. Unlike the base language where every expression terminates, we now introduce the possibility of runtime errors (e.g., division by zero). The fundamental change is that expressions may either evaluate to a value \textit{or} raise an exception.
...
data ⊢_ where
    `_              : ⊨ T → ⊢ T
    _⊞_             : ⊢ Nat → ⊢ Nat → ⊢ Nat
    _/_             : ⊢ Nat → ⊢ Nat → ⊢ Nat  -- Division (may raise)
    test            : ⊢ Nat → ⊢ Bool
    if_then_else_   : ⊢ Bool → ⊢ T → ⊢ T → ⊢ T
    -- Exception handling
    throw           : Exception → ⊢ T
...
\section{Big-Step Semantics with Exceptions}
...
data _⇓ʳ_ : ⊢ T → Result T → Set where
    ...
    -- Arithmetic: propagate exceptions
    _⊞_ 
      : e₁ ⇓ʳ r₁ → e₂ ⇓ʳ r₂ 
      → (e₁ ⊞ e₂) ⇓ʳ (liftA2 (+) r₁ r₂)
...
data _⇾_ : ⊢ T → ⊢ T → Set where
    ...
    β-/-zero : (` n / ` 0) ⇾ throw err
    β-/ : n₂ ≠ 0 → (` n₁ / ` n₂) ⇾ ` (n₁ / n₂)
    
    -- Exception propagation: if subterm throws, bubble it up
    ξ-⊞₁-exc : e₁ ⇾ throw err → (e₁ ⊞ e₂) ⇾ throw err
    ξ-⊞₂-exc : e₂ ⇾ throw err → (` n ⊞ e₂) ⇾ throw err
    
    -- Try-catch handling
    ξ-try : e ⇾ e' → (try e catch h) ⇾ (try e' catch h)
    β-try-throw : (try (throw err) catch h) ⇾ (h err)
\end{minted}
...
\section{Abstract Machine with Exception Handling}
