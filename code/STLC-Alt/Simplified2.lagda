...
module STLC-Alt.Simplified2 where
...
data Frame : Type → Type → Set where
  ...
  app₂    : ⊨ (T₁ ⇒ T₂) → Hole  → Frame T T₁
  app₃    : ⊨ (T₁ ⇒ T₂) → ⊨ T₁  → Hole → Frame T T₂
\end{code}
...
data State (Answer : Type) : Set where
  _⊢_↓_ : {Γ : Context} → (δ : Environment Γ) → Stack Answer T → ⊨ T → State Answer
...
data _⇾_ : State Answer → State Answer → Set where
  step-`  : ∀ {stack : Stack Answer A} {v : ⊨ A} → (δ ⊢ stack ↑ ` v) ⇾ (δ ⊢ stack ↓ v) 
  ...
  step-·₁ : ∀ {δᶜ : Environment Δ} {eᶜ : (Δ , T₁) ⊢ T₂} {e₂ : Γ ⊢ T₁} {stack : Stack Answer T₂}
    → (δ ⊢ stack ∷ app₁ ◌ e₂ ↓ ⟨ δᶜ , ? ⟩) ⇾ (δ ⊢ stack ∷ app₂ ⟨ δᶜ , eᶜ ⟩ ◌ ↑ e₂)
  
  step-·₂ : ∀ { stack : Stack Answer T₂ } { δᶜ : Environment Δ } { eᶜ : ( Δ , T₁ ) ⊢ T₂ } { v₂ : ⊨ T₁ }            
    → (δ ⊢ stack ∷ app₂ ⟨ δᶜ , eᶜ ⟩ ◌ ↓ v₂) ⇾ ((δᶜ , v₂) ⊢ stack ∷ app₃ ⟨ δᶜ , eᶜ ⟩ v₂ ◌ ↑ eᶜ) 

  step-·₃ : -- ∀ {stack} {v₁ : ⊨ (A ⇒ B)} {v₂ : ⊨ A}
    ∀ {stack : Stack Answer T₂} {δᶜ : Environment Δ} {eᶜ : (Δ , T₁) ⊢ T₂} {v₂ : ⊨ T₂} {v₁ : ⊨ T₁} 
    → ((δᶜ , v₁) ⊢ stack ∷ app₃ ⟨ δᶜ , eᶜ ⟩ v₁ ◌ ↓ v₂) ⇾ (δ ⊢ stack ↓ v₂) 
\end{code}
...
-- total (δ ⊢ stack ∷ app₂ ⟨ δᶜ , eᶜ ⟩ ◌ ↓ v) ¬final = {!   !} ﹐ {!   !}
...
complete
  : ∀ (δ : Environment Γ) (stack : Stack Answer A) (e : Γ ⊢ A) 
  → ∀ (v : ⊨ A) (p : δ ⊢ e ⇓ v) 
...
_⊢_[_] : ∀ {Γ : Context} → Environment Γ → Stack Answer A → Γ ⊢ A → Γ ⊢ Answer
δ ⊢ [] [ e ] = e
...
⪅-compositional r (s ∷ app₁ x x₁) = ⪅-compositional {!!} {!s!}
⪅-compositional r (s ∷ app₂ x x₁) = ⪅-compositional {!!} {!s!}
⪅-compositional r (s ∷ app₃ δ x x₁ x₂) = ⪅-compositional {!!} {!!}
\end{code}
