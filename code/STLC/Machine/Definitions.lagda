...
module STLC.Machine.Definitions where
...
data Frame : Context → Context → Type → Type → Set where
  ...
  -- app₃: outer context is Γ (with environment δ), but we're evaluating the closure body in Δ
  -- We store the outer environment so we can restore it when returning.
  app₃    : Environment Γ → ⊨ (A ⇒ B) → ⊨ A → Hole → Frame Γ Δ B B
\end{code}
...
data State (Answer : Type) : Set where
  ...
  _⊢_↓_ : (δ : Environment Δ) → Stack Answer Γ Δ T → ⊨ T → State Answer
  _⊢_↑_ : (δ : Environment Δ) → Stack Answer Γ Δ T → Δ ⊢ T → State Answer
\end{code}
